"use client";

import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { InvoiceSchema, InvoiceInput } from "../validations/invoiceSchemas";
import { FormField } from "@/components/forms/FormField";
import { Button } from "@/components/ui/button";
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog";

type InvoiceFormProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (data: InvoiceInput) => Promise<void>;
  defaultValues?: Partial<InvoiceInput>;
};

export function InvoiceForm({ open, onOpenChange, onSubmit, defaultValues }: InvoiceFormProps) {
  const form = useForm<InvoiceInput>({
    resolver: zodResolver(InvoiceSchema),
    defaultValues: defaultValues || { type: "INVOICE", status: "DRAFT" },
  });

  async function handleSubmit(data: InvoiceInput) {
    await onSubmit(data);
    onOpenChange(false);
    form.reset();
  }

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Factura</DialogTitle>
        </DialogHeader>
        <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
          <FormField label="Numero" {...form.register("number")} error={form.formState.errors.number?.message} />
          <FormField label="Cliente" {...form.register("clientId")} error={form.formState.errors.clientId?.message} />
          <FormField label="Serie" {...form.register("series")} error={form.formState.errors.series?.message} />
          <div className="flex justify-end gap-2">
            <Button type="button" variant="secondary" onClick={() => onOpenChange(false)}>
              Cancelar
            </Button>
            <Button type="submit">Guardar</Button>
          </div>
        </form>
      </DialogContent>
    </Dialog>
  );
}